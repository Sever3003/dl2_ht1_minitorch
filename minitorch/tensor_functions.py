...
class Sigmoid(Function):
    ...
    def backward(ctx: Context, grad_output: Tensor) -> Tensor:
        ...
        ones = sigma.f.eq_zip(sigma, sigma)
        one_minus_sigma = sigma.f.add_zip(ones, sigma.f.neg_map(sigma))
        return grad_output.f.mul_zip(grad_output, sigma.f.mul_zip(sigma, one_minus_sigma))
...
class Permute(Function):
    ...
    def forward(ctx: Context, a: Tensor, order: Tensor) -> Tensor:
        # TODO: Implement for Task 2.3.
        order = [int(order[i].item()) for i in range(order.size)]
        ctx.save_for_backward(tuple(order))
