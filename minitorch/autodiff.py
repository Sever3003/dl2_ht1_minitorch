...
def central_difference(f: Any, *vals: Any, arg: int = 0, epsilon: float = 1e-6) -> Any:
    ...
    fp_vals = list(vals)
    fp_vals[arg] += epsilon
    return (f(*fp_vals) - f(*f_vals)) / epsilon
...
def topological_sort(variable: Variable) -> Iterable[Variable]:
    ...
    visited = set()
    topology = []
    def dfs(val: Variable) -> None:
        if (val.unique_id in visited) or (val.is_constant()):
            return
...
def backpropagate(variable: Variable, deriv: Any) -> None:
    ...
    for var in topological_sort(variable):
        grad_output = gradients.get(var.unique_id, 0.0)

        if var.history is None or var.history.last_fn is None:
            continue

        for parent, grad in var.chain_rule(grad_output):
